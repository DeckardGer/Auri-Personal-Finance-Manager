...
export const processTransactions = async (file: File) => {
  ...
  const dateColumn =
    userSettings.dateColumnIndex ?? firstRow.findIndex((column) => column.toLowerCase() === 'date');
  const amountColumn =
  ...
  if (dateColumn === -1 || amountColumn === -1 || descriptionColumn === -1) {
    throw new Error('Date, amount, or description column not found');
  }
  ...
  const uploadedCSVTransactions: CSVTransaction[] = text
    ...
    .map((line) => {
      ...
      return {
        date: new Date(transaction[dateColumn]),
        amount: Number(transaction[amountColumn]),
        description: transaction[descriptionColumn],
      };
    });
